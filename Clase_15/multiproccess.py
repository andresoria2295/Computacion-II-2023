"""
2 - Realizar dos versiones de un servidor de mayúsculas que atienda múltiples clientes de forma
concurrente utilizando multiprocesamiento y threading utilizando sockets TCP.
El hilo/proceso hijo debe responder con mayúsculas hasta que el cliente envíe la palabra exit.
En caso de salida el cliente debe gestionar correctamente el cierre de la conexión y del proceso/hilo.
"""

#!/usr/bin/python3
import socket
import multiprocessing

def mpserver(cliente):
    sock, addr = cliente
    try:
        while True:
            msg_cliente = sock.recv(1024)
            print("Mensaje recibido: %s de %s" % (msg_cliente.decode(), addr))
            decoded_original = msg_cliente.decode()
            #Strip() elimina espacios en blanco alrededor
            decoded = msg_cliente.decode().strip()
            if decoded == "exit":
                print("Cerrando conexión con %s\n" % str(addr))
                sock.send("Conexión finalizada.".encode('utf-8'))
                sock.close()
                #Cerrar la conexión para finalizar este proceso hijo
                #terminate()
                break
            else:
                msg_server = decoded_original.upper()
                sock.send(msg_server.encode('utf-8'))
    except:
        print("Error en la conexión con %s" % str(addr))
        sock.close()

def main():
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    s.bind(('0.0.0.0', 50010))
    s.listen(5)
    procesos = []
    print("Servidor en linea.")
    try:
        while True:
            cliente = s.accept()
            conn_client, address = cliente
            print("\nConexión establecida con %s" % str(address))
            msg = 'Conexión exitosa: Hola! Soy el servidor. ¿En qué puedo ayudarte?.\n'
            print("Enviando mensaje al cliente...")
            conn_client.send(msg.encode('utf-8'))
            child = multiprocessing.Process(target=mpserver, args=(cliente,))
            child.start()
            procesos.append(child)

    except KeyboardInterrupt:
        print("\nCerrando conexiones...")
        for p in procesos:
            #Terminar hijo
            #p.terminate()
            #Espera a que el hijo termine.
            p.join()

        s.close()
        print("Servidor cerrado.")

if __name__ == '__main__':
    main()
